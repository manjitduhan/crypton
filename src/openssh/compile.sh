...
build() {
  local source_dir="$SOURCE_DIR" generated_source
  ...
  [[ -d "$OUTPUT_ROOT/openssl" ]] || die 'OpenSSL must be built before OpenSSH'
  # OpenSSH links against the isolated OpenSSL prefix rather than the host
  # installation, and embeds an rpath so the binary finds that same copy.
  configure_args=(
    --prefix="$OUTPUT_DIR"
    --sysconfdir="$OUTPUT_DIR/etc/ssh"
    --with-ssl-dir="$OUTPUT_ROOT/openssl"
    --with-privsep-path="$OUTPUT_DIR/var/empty"
    --with-pid-dir="$OUTPUT_DIR/var/run"
  )
  (cd "$BUILD_DIR" && LDFLAGS="-Wl,-rpath,$OUTPUT_ROOT/openssl/lib:$OUTPUT_DIR/lib" "$source_dir/configure" "${configure_args[@]}" "$@")
...
run_server() {
  ...
  local config="$OUTPUT_DIR/etc/ssh/sshd_config" runtime="$OUTPUT_DIR/var/run"
  ...
  [[ -x "$OUTPUT_DIR/sbin/sshd" ]] || die 'OpenSSH is not built; run ./crypton build openssh'
  ...
  [[ "$command_line" == *"$OUTPUT_DIR/sbin/sshd"* ]] || die "refusing to accept unexpected OpenSSH PID $pid"
...
run_client() {
  ...
  [[ -x "$OUTPUT_DIR/bin/ssh" ]] || die 'OpenSSH is not built; run ./crypton build openssh'
  "$OUTPUT_DIR/bin/ssh" "${options[@]}" "$@"
}

stop_server() {
  # Verify the PID command line before sending a signal to avoid affecting a
  # system OpenSSH service or another user's process.
  local pid_path="$OUTPUT_DIR/var/run/sshd.pid" pid command_line attempt
  ...
  command_line=$(ps -p "$pid" -o args=)
  [[ "$command_line" == *"$OUTPUT_DIR/sbin/sshd"* ]] || die "refusing to stop unexpected process $pid"
  kill -TERM "$pid"
...
run() {
  ...
  [[ -x "$OUTPUT_DIR/bin/ssh" && -x "$OUTPUT_DIR/sbin/sshd" ]] || die 'OpenSSH is not built; run ./crypton build openssh'
  if (( $# == 0 )); then run_server; return; fi
  case "$1" in
    server) shift; run_server "$@" ;;
    client) shift; (( $# == 0 )) && { "$OUTPUT_DIR/bin/ssh" -V; return; }; run_client "$@" ;;
    *) run_client "$@" ;;
  esac
}
